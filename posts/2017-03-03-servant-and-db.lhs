...
`servant` offers the [`enter`](https://hackage.haskell.org/package/servant-0.10/docs/Servant-Utils-Enter.html#v:enter) utility, which let's you to remove this kind of
...
`H` into servant's `Handler`. The `enter` machinery walks thru `Server`-like
type and applies that transformation, our endpoints are instantiated to `H` in
the process, so everything works out nicely.

If we used `ReaderT (Connection Pool) Handler` as our concrete monad,
then we could use `runReaderTNat` to construct `nt`.

The `enter` is most useful when you have polymorphic handlers defined with
...
handler, and `ReaderT (Pool Connection) Handler` in some other one, `enter`
won't help!
...
```
fetch the source from [https://gist.github.com/phadej/4757105da52c1a1c946abe3b204ec500](https://gist.github.com/phadej/4757105da52c1a1c946abe3b204ec500)
